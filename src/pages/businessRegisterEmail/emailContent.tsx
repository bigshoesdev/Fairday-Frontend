...
const EmailContent = ({bufferEmail}) => {
  ...
  const decodedIdentify = BusinessProfileConfig.decodedIdentify;
      ...
      </div>
      {
        decodedIdentify ? (<>
          <h2 className="text-center title mt-4">
            Congratulations Your Business Services for Hire are registered with Fariday
          </h2>
          ...
          <span className="text-primaryBlue title">BHP # {decodedIdentify}</span>
          <span onClick={() => navigate('/business-profile')} className="font-bold text-black cursor-pointer hover:text-blue-500">VIEW  PROFILE</span>
          ...
          <h2 className="text-center title mt-4">
            Please verify your email {bufferEmail} to complete your business profile registration for Fariday Jobs. Check your inbox for a verification link
          </h2>
        </>)
      }
