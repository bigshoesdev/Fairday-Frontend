...
const PreviewModal: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const { adDetails } = advertisementConfig;

  const imagesPath = Array.isArray(adDetails?.result?.images) ? adDetails.result.images : [];
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-[100] ">
      ...
      >
        <div className="flex justify-between items-center px-8">
          <span className="text-[18px] text-white font-semibold">Advertisement Preveiw</span>
        ...
        <div className="bg-white rounded-lg py-5 px-8 flex flex-col">
          ...
          <div className="mt-4 flex flex-row gap-4">
            ...
            <div className="flex flex-col  justify-between">
              <span className="text-blue-400 font-bold">{adDetails?.result?.businessName || ''}</span>
              <span className="text-black font-bold">{adDetails?.result?.street || ''}</span>
            </div>
          </div>
          ...
          <div className="flex justify-between items-end mt-3">
            ...
            {adDetails?.result?.insertWebsite || ''}
            </span>
            <div className="flex flex-row ml-6">
            {adDetails?.result?.promoEndDate || ''}
            </div>
          </div>
          ...
          <div className="flex justify-between mt-4">
            ...
            <span className="text-gray-500 text-[15px] font-medium">{dayjs(adDetails?.result?.createdAt).fromNow() || ''}</span>
          </div>
