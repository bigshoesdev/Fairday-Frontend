...
const CategorySelect = ({ businessProfileValue, bufferSetBusinessProfileValue }) => {
  ...
  const handleItemClick = (itemId: string, itemName: string) => {
    const selectedCategories = businessProfileValue.selectedCategories || [];
  ...
  const handleRemoveCategory = (itemId: string) => {
    bufferSetBusinessProfileValue({
      ...businessProfileValue,
      selectedCategories: businessProfileValue.selectedCategories.filter(id => id !== itemId),
    });
  };
        ...
        header={
          <div className='flex flex-col gap-2'>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {businessProfileValue.selectedCategories?.map((categoryId) => {
              ...
              <ul className="space-y-1">
                {groupedData[letter].map((item: { id: string; name: string }) => (
                  <li
                    ...
                    className={`cursor-pointer ${businessProfileValue.selectedCategories?.includes(item.id) ? 'text-blue-500 font-bold' : 'text-gray-700'
                      } hover:text-blue-500`}
                  >
                    {item.name}
                  </li>
